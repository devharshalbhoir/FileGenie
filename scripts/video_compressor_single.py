...
def is_candidate(meta, target_height=480):
    if not meta:
        return False
    # If already smaller or equal to target height, usually skip (unless user wants to re-compress)
    ...
    if meta["size_mb"] <= 50:
        return False
    return (
        (meta["duration"] < 420 and meta["size_mb"] > 90) or
        meta["mb_per_min"] > 15
    )
...
def build_ffmpeg_cmd(src, dst, args):
    # scale filter
    scale_filter = f"scale=w={args.scale_width}:h={args.scale_height}:force_original_aspect_ratio=decrease"
    ...
    if not args.dry_run:
        if dst.exists():
            ...
            log_func(
             ...
             log_func(f"ERROR: {rel_path} - Destination file not created.\n")
    else:
